...
class InferenceHandler3DEPN:
    ...
    def infer_single(self, input_sdf, target_df):
        ...
        input_sdf = np.clip(input_sdf, a_max = self.truncation_distance, a_min = -self.truncation_distance)
        target_df = np.clip(target_df, a_max = self.truncation_distance, a_min = 0)
        with torch.no_grad():
            ...
            input_sdf_cat = np.concatenate((distances[None,:,:,:], sign[None,:,:,:]),axis = 0)
            
            reconstructed_df = self.model(torch.from_numpy(input_sdf_cat)[None,:,:,:,:].double())
