...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            batch_indices = torch.randint(0,points.shape[0],(int(self.num_samples), ))
            batch_points = torch.index_select(points,0,batch_indices)
            batch_sdf = torch.index_select(sdf,0,batch_indices)
            ...
            predicted_sdf = model(torch.cat((latent_codes, batch_points),1))
