...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        a,b=pos_tensor.shape
        index = torch.randint(0,a,(int(self.num_sample_points/2), )) 
        pos_tensor =torch.index_select(pos_tensor,0,index)
        a,b=neg_tensor.shape
        index= torch.randint(0,a,(int(self.num_sample_points/2),)) 
        neg_tensor =torch.index_select(neg_tensor,0,index)

        sdf=torch.cat((pos_tensor,neg_tensor),0) 
        return sdf
        # a, b = pos_tensor.shape
        # index =  np.random.choice(a, int(self.num_sample_points/2))
        # pos_tensor = pos_tensor[index]

        # a, b = neg_tensor.shape
        # index = np.random.choice(a, int(self.num_sample_points/2))
        # neg_tensor = neg_tensor[index]

        # sdf = np.concatenate((pos_tensor,neg_tensor),0)
