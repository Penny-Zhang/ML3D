...
def procrustes_align(pc_x, pc_y):
    ...
    mean_x = np.average(pc_x, axis = 0)
    mean_y = np.average(pc_y, axis = 0)
    
    centered_x = pc_x - mean_x
    centered_y = pc_y - mean_y
    
#     X = np.reshape(centered_x, (3, len(pc_x)))
#     Y = np.reshape(centered_y, (3, len(pc_y)))
    X = np.transpose(centered_x)
    Y = np.transpose(centered_y)
    
    U, D, VT = np.linalg.svd(np.dot(X, np.transpose(Y)))
    R = np.dot(np.transpose(VT), np.transpose(U))
    
    x = np.reshape(mean_x, (3,))
    y = np.reshape(mean_y, (3,))
    t = y - np.dot(R, x) 
