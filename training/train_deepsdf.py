...
def train(model, latent_vectors, train_dataloader, device, config):
    ...
    # def loss_criterion(predicted_sdf,sdf):
    #     loss_criterion = torch.abs(torch.clamp(predicted_sdf ,max = config['lambda_code_regularization'], min = -config['lambda_code_regularization']) - torch.clamp(sdf ,max = config['lambda_code_regularization'], min = -config['lambda_code_regularization']))
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # predicted_sdf[predicted_sdf < -0.1] = -0.1 
            # predicted_sdf[predicted_sdf > 0.1] = 0.1 
